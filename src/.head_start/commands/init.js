...
const { exitProcess, boolean} = require("yargs");
const { exec, execSync } = require('child_process');
const {Question} = require("./functions/txtutils");
...
exports.handler = function (argv) {
  if (argv["welcome"] === true) {
    ...
    o.Line("During this walk through, we are going to guide you through the process of setting up:");
    o.List([
  ...
  let dot_directory = "";
  if (argv["d"] === ".") {
    o.Line("You've specified your current working directory as your project root. Is this correct?")
    o.Empty();
    o.Line(path.resolve(process.cwd()));
    if (o.Question("^ (Y/n) ", "y").toLowerCase() === "n") {
      // No
      project_directory = path.resolve(o.Question("Which directory would you like to use instead? "));
    } else {
      // Yes
      project_directory = path.resolve(process.cwd());
    }
  } else {
    o.Line("Below is the directory you've specified for your project root. Is this correct?")
    o.Empty();
  ...
  if (!fs.existsSync(project_directory)) {
    ...
    try {
      ...
      o.Error("There was an error with the project folder creation, and this tool cannot continue running.");
      o.Error(err);
      return 1;
    }
    ...
    try {
      ...
      o.Error("There was an error with the necessary file creation, and this tool cannot continue running.");
      o.Error(err);
      return 2;
    }
  }
  ...
  execSync("sh " + path.resolve(__dirname) + "/setup_npm.sh " + project_directory + " " + dot_directory, (err, stdout, stderr) => {
    if (err) {
      //some err occurred
      o.Error(err);
      o.Line("Unknown framework. Leave the line blank of type  none  if you aren't using one.")
  ...
  let framework_prompt = () => {
    ...
    if (framework.length > 0 && framework !== "none") {
      ...
      exec("sh " + path.resolve(__dirname) + "/add_framework.sh "
                 + project_directory + " " /* Argument #1 */
                 + dot_directory + " "     /* Argument #2 */
                 + framework + " "         /* Argument #3 */
...
function exec_error(err, stdout, stderr) {
  if (err) {
    //some err occurred
    o.Error(err);
    return 3;
